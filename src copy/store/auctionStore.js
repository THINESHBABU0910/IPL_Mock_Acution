...
export const useAuctionStore = create()(
    persist(
        (set, get) => ({
            ...
            connect: (userName, roomCode, action = 'JOIN') => {
                ...
                socket.onmessage = (event) => {
                    ...
                    if (data.type === 'ROOM_CREATED' || data.type === 'JOIN_SUCCESS') {
                        const { room } = data;
                        set({
                            room: { code: room.code, creator: room.creator },
                            teams: room.teams,
                    ...
                    if (data.type === 'STATE_UPDATE') {
                        ...
                        set(state => ({
                            room: { ...state.room, status: room.status, isPaused: room.isPaused },
                            teams: room.teams,
                            currentPlayerIndex: room.currentPlayerIndex,
                            currentBid: room.currentBid,
                            timerDuration: room.timerDuration,
            ...
            createRoom: (userName) => {
                get().connect(userName, null, 'CREATE');
            },
            ...
            selectTeam: (teamName) => {
                if (socket && socket.readyState === 1) {
                    socket.send(JSON.stringify({ type: 'SELECT_TEAM', team: teamName }));
                }
            },
