...
export default function Landing({ onNavigate }) {
    ...
    const handleAction = () => {
        if (!name.trim()) return alert('Identify yourself, Commander.');

        if (mode === 'create') {
            createRoom(name);
            onNavigate('team-selection');
        } else {
            if (!roomCode.trim()) return alert('Enter a Room Code to join.');
    ...
    const handleResume = (room) => {
        if (!name.trim()) {
            const n = prompt("Enter your name to rejoin:", name) || "Guest";
            setName(n);
            joinRoom(room.code, n);
        } else {
            joinRoom(room.code, name);
        }
        // Check if user already has a team in this room
        // The store update is async, but usually fast enough. 
        // If they have a team, Auction will render. If not, Auction redirects to TeamSelection.
        // However, to be safe, we can default to TeamSelection. 
        // Auction component logic: if (!currentUser.team) -> navigate team-selection.
        // So sending to 'auction' is fine, it will redirect if needed.
        onNavigate('auction');
    };
